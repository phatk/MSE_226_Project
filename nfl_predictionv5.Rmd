...
NFL_Train_Intuitive_Model$coefficients
```
...
finding the NFL player that is significant from rdige lambda
```{r}
player_id <- filter(NFL_DATA, NflIdRusher == 2558208) 
```
Chad Williams
...
#Logistic Regression
```{r}
glm_fit <- glm(FirstDown ~ ., data = knn_training, family = binomial)
summary(glm_fit)
```
...
# SVM
```{r}
svm <- svm(FirstDown ~ ., data = knn_training, kernel="radial", cost = 1, gamma = 0.01)
summary(svm)
```
...
knn.pred <- knn(train = knn_training, test = knn_validation, cl = knn_training_labels, k = 5)

trctrl <- trainControl(method = "cv", number = 5)
set.seed(3333)
knn_fit <- train(FirstDown ~., data = NFL_DATA_TRAIN_Filtered_Final, method = "knn",
 trControl=trctrl,
 tuneLength = 3)
